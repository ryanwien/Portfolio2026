...
- `matching_engine/order.py` — `Order` (also a list node), `Trade`, enums
- `matching_engine/book.py` — `PriceLevel` and the `OrderBook` engine
- `tests/test_book.py` — matching, FIFO priority, cancels, market orders, validation
